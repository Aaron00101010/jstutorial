...
performance.now方法返回当前网页自从performance.timing.navigationStart到当前时间之间的毫秒秒数（精度可达到微秒）。也就是说，它的精度可以达到100万分之一秒。
...
上面代码表示，performance.timing.navigationStart加上performance.now()，近似等于Date.now()，也就是说，Date.now()可以替代performance.now()。但是，两者返回的是毫秒，所以后者的精度比前者高1000倍。
...
上面代码返回第一个HTTP请求（即网页的HTML源码）的时间统计信息。该信息以一个高精度时间戳的对象形式返回，每个属性的单位是
秒（microsecond），即百万分之一秒。
